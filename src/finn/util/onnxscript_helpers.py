    ...
    for node in graph._nodes:
        if node.op_type == "Constant" and not node.metadata_props:
            ...
            for output in node.outputs:
                for user in output.uses():
                    ...
                    if user_node.metadata_props:
                        metadata.add(
                            (
                                user_node.metadata_props["pkg.torch.onnx.name_scopes"],
            ...
            if len(metadata) == 1:
                ...
                metadata_list = [
                    (ast.literal_eval(x[0]), ast.literal_eval(x[1])) for x in list(metadata)
                ]
...
def normalize_io_for_loop_rolling(graph, LoopBody):
    ...
    for index, LoopInputType in enumerate(LoopBody.signature):
        ...
        if (
            LoopInputType == LoopBodyInputType.CONSTANT
            or LoopInputType == LoopBodyInputType.PARAMETER
        ):
            ...
            Ident = ir.Node(
                domain="", op_type="Identity", inputs=[cinput], outputs=[noutput], num_outputs=1
            )
    ...
    iteration = ir.Value(
        name="iteration", shape=ir.Shape([1]), type=ir.TensorType(ir.DataType.INT64)
    )
    ...
    condition_ident = ir.Node(
        domain="", op_type="Identity", inputs=[condition], outputs=[condition_out], num_outputs=1
    )
...
def build_loop_replace_pattern(graph, LoopBody):
    ...
    for i, LoopInputType in enumerate(LoopBody.signature):
        if LoopInputType == LoopBodyInputType.PARAMETER:
            ...
            n_constant_node = build_constant_from_tensor(
                constant_input.name + "_const_val", ir.Tensor(constant_value)
            )
    ...
    graph_nodes.append(
        ir.Node(
            "", "Loop", inputs=loop_inputs, attributes=[body_attr], outputs=loop_outputs, graph=None
        )
    )
